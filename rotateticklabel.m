function TextHandle = rotateticklabel(ha,tag,rot,FontSize,position)
%   ???????????????????????????????
%   ha   ????????????????????????????????????????
%   tag  ???????????????????????('X'???'Y')???????????X?????????
%   rot  ??????????????????????
...
if strncmpi(tag,'x',1)
    if isempty(oldxticklabel)
        ...
        set(ha,'XTickLabel',[]);
        if position==1
            y = zeros(size(x)) + yl(1) - range(yl)/100;
        else
            y = zeros(size(x)) + yl(end) + range(yl)/100;
        end
    ...
    if isempty(oldyticklabel)
        ...
        xl = xlim(ha);
        set(ha,'YTickLabel',[]);
        x = zeros(size(y)) + xl(1) - range(xl)/70;
        TextHandle = text(x,y,str,'rotation',rot,...
...
rot = mod(rot,360);
if rot>=0 && rot<180
%    set(TextHandle,'HorizontalAlignment','right', 'FontSize', 6, 'FontWeight','bold');
   set(TextHandle,'HorizontalAlignment','right', 'FontSize', FontSize);
else
%    set(TextHandle,'HorizontalAlignment','left', 'FontSize', 6, 'FontWeight','bold');
   set(TextHandle,'HorizontalAlignment','left', 'FontSize', FontSize);
end
